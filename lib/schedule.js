...
// Every valid 1-hour booking start time in a day, in minutes-from-midnight:
// every hour on the hour, PLUS the half-hour after it UNLESS that would run
// past closing time (the half-hour slot starting at the last hour would end
// 30 minutes after the business already closes, so it's excluded).
export function validSlotStarts(hours) {
  const lastHour = Math.max(...hours);
  const starts = [];
  hours.forEach(h => {
    starts.push(h * 60);
    if (h < lastHour) starts.push(h * 60 + 30);
  });
