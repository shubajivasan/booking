...
// Sends the booking confirmation to the STUDENT who made the booking,
// separate from the internal admin alert above — different tone, different
// recipient, same underlying Gmail connection.
export async function sendStudentConfirmation({ to, subject, html }) {
  const user = process.env.GMAIL_USER;
  const t = getTransporter();

  if (!t || !to) {
    console.warn('Student confirmation email skipped — GMAIL_USER/GMAIL_APP_PASSWORD not set, or no student email given.');
    return;
  }

  try {
    await t.sendMail({
      from: `"Ajivasan Academy of Performing Arts" <${user}>`,
      to,
      subject,
      html,
    });
  } catch (err) {
    console.error('Gmail SMTP send to student failed:', err);
  }
}
